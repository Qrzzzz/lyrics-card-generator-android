...
- Renderer 在第三步按需初始化并在后续步骤与导出间复用；步骤一、二不创建预览，本地字体与静态资源可缓存；
- 预览更新采用 latest-wins 串行队列，首次内容不再等待自动测高，连续修改以最后一次设置为准；
- Xiaomi 2210132C（Android 16 / API 36、System WebView 149）真机验证：前两步无 WebView，第三步首次显示卡片，横竖版连续切换与步骤往返均保留最终预览；
