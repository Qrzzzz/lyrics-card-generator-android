...
仅允许 `/renderer/` 与 `/media/`，其余请求返回 403。WebView 禁用文件/内容访问、DOM Storage、混合内容、弹窗、新窗口、下载和外部导航。应用不声明 `INTERNET` 权限。
...
封面由系统 Picker 选取后复制到应用私有目录。`ProjectAssetStore` 限制输入大小，并只接受 UUID 路径。渲染器通过 `/media/<assetId>` 读取，不接触真实文件路径。
