...
- 修复 #74：实际横版画布高度使用 640..6400，手动 requestedHeight 保持 720..3600；Kotlin、Schema/TypeScript 与桥接响应一致，不补高画布，不移除非法/超大尺寸校验。
- 修复 #73：当前项目、撤销/重做历史、导入回滚快照和在途保存明确持有封面文件；历史失效与编辑器关闭后按数据库共享引用回收。取消或失败导入释放未提交文件。
- 修复 #76：失败导入按 revision 仅撤回仍由导入拥有的字段，保留后续歌词、排版、项目名和编辑历史；替换导入等待旧操作完成回滚。
- 版本 2.1 / 20100；兼容 2.0 保存项目，包名、签名和 renderer 合同不变。未加入新功能。

## 2.0 — 文档与渲染升级

- 采用 LyricDocumentV2，统一段落、稳定 ID、原译文配对和空行；编辑、撤销重做、项目保存与渲染共享文档。1.x 项目明确不兼容，保留原数据，不自动迁移。
- 对齐桌面 v6.3.5：横竖自动尺寸、DOM 内容测量、字体预设与本地字体导入、中西文混排、间距、空间色场和网格；保留六步原生界面。
- 支持 PNG / WebP / JPG 和 1× / 1.4× / 2×，统一预览与导出尺寸，保留原生保存分享并校验格式字节。
- 新版本使用 2.0、2.1 两段号，versionCode 从 10107 增至 20000。生产签名、来源校验与 focused-manual-v1 流程不变；正式候选及发布授权见 [2.0 验收记录](docs/releases/v2.0-acceptance.json)。
...
## 1.1.5 — 安全依赖与 API 30 回归

- 修复 Protobuf、Guava 和 jsoup 的构建／测试依赖告警。
- 正式签名候选的 API 30 serif 1×／2×、取消清理重试和 ATF 定向回归通过；强制真机人工验收改为可选。

## 1.1.4 — 导出维护整理

- 独立整理导出分块写入与临时文件清理，沿用现有图片导出、取消和重试行为。
- 保持已保存项目、图片样式及分享方式兼容。
- 本版按维护者要求跳过六项人工验收，结果为 NOT RUN；自动检查和签名来源验证独立执行。
